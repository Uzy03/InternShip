...
def verify_final_100_percent():
    """最終統一後の100%町丁一貫性率達成の検証"""
    print("=== 最終統一後の100%町丁一貫性率達成の検証開始 ===")
    
    # ディレクトリ設定
    csv_dir = Path("../Preprocessed_Data_csv")
    
    # 最終統一済みファイルを取得
    csv_files = list(csv_dir.glob("*_100percent_working.csv"))
    csv_files.sort()
    
    if not csv_files:
        print("❌ 最終統一済みファイルが見つかりません")
        return
    ...
    # 最終検証結果
    verification_file = output_dir / "final_100_percent_verification_result.txt"
    with open(verification_file, 'w', encoding='utf-8') as f:
        f.write("=== 最終統一後の100%町丁一貫性率達成の検証結果 ===\n")
...
def main():
    """メイン処理"""
    consistency_rate, consistent_towns, all_towns = verify_final_100_percent()
