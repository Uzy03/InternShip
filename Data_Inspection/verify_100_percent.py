...
def verify_100_percent():
    """100%町丁一貫性率達成の検証"""
    print("=== 100%町丁一貫性率達成の検証開始 ===")
    
    # ディレクトリ設定
    csv_dir = Path("../Preprocessed_Data_csv")
    
    # 100%統一済みファイルを取得
    csv_files = list(csv_dir.glob("*_100percent.csv"))
    csv_files.sort()
    
    if not csv_files:
        print("❌ 100%統一済みファイルが見つかりません")
        return
    ...
    # 100%検証結果
    verification_file = output_dir / "100_percent_verification_result.txt"
    with open(verification_file, 'w', encoding='utf-8') as f:
        f.write("=== 100%町丁一貫性率達成の検証結果 ===\n")
    ...
    print(f"\n=== 検証結果保存完了 ===")
    print(f"✓ 100%検証結果: {verification_file}")
    
    return consistency_rate, consistent_towns, all_towns

def main():
    """メイン処理"""
    consistency_rate, consistent_towns, all_towns = verify_100_percent()
