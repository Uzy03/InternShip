...
# -*- coding: utf-8 -*-
"""
100%町丁一貫性率達成のための最終統一スクリプト
区名付き町丁を区名なしに統一して完全な一貫性を実現
"""
...
def unify_for_100_percent():
    """100%町丁一貫性率達成のための最終統一処理"""
    print("=== 100%町丁一貫性率達成のための最終統一処理開始 ===")
    ...
    for csv_file in csv_files:
        ...
        try:
            ...
            for idx, town_name in enumerate(town_col):
                ...
                # 最終統一対象町丁かチェック
                if town_name in final_district_towns:
                    # 区名なしに統一
                    df.iloc[idx, 0] = normalize_to_base_town(town_name)
                    changes_made += 1
                    print(f"  → 最終統一: {town_name} → {normalize_to_base_town(town_name)}")
            
            if changes_made > 0:
                print(f"  → 変更件数: {changes_made}件")
            
            # 出力ファイル名
            output_filename = f"{csv_file.stem}_100percent.csv"
