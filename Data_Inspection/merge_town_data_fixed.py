...
def identify_merged_towns():
    ...
    merged_towns = {
        '城南町系': [
            ...
            '城南町七滝', '城南町鰐瀬', '城南町上', '城南町下', '城南町上益城',
            '城南町下益城', '城南町上益城郡', '城南町下益城郡'
        ],
        '益城町系': [
            '益城町小池', '益城町惣領', '益城町福原', '益城町木山', '益城町寺迫',
            '益城町杉堂', '益城町御船', '益城町田原', '益城町福富', '益城町上陳',
            '益城町下陳', '益城町広崎', '益城町赤井', '益城町安永', '益城町山崎',
            '益城町上馬田', '益城町下馬田', '益城町上馬田郡', '益城町下馬田郡'
        ],
        '嘉島町系': [
            '嘉島町上六嘉', '嘉島町下六嘉', '嘉島町上六嘉郡', '嘉島町下六嘉郡'
        ],
        '西原村系': [
            '西原村小森', '西原村河原', '西原村小森郡', '西原村河原郡'
        ]
    }
...
def get_merge_year(era_year):
    ...
    if era_year.startswith('H'):
        year = int(era_year[1:3]) + 1988
    elif era_year.startswith('R'):
        year = int(era_year[1:3]) + 2018
    else:
        return None
...
def extract_population_data_batch(df, town_names):
    ...
    for i, row in df.iterrows():
        ...
        if current_town and current_value in age_columns:
            try:
                ...
                male_str = str(row.iloc[1]).replace(',', '') if pd.notna(row.iloc[1]) else '0'
                female_str = str(row.iloc[2]).replace(',', '') if pd.notna(row.iloc[2]) else '0'
    ...
    for town in town_names:
        if town not in population_data_dict:
            population_data_dict[town] = {
                'total': 0,
                'male': 0,
                'female': 0,
            ...
            for age_col in age_columns:
                if age_col not in population_data_dict[town]['age_data']:
                    population_data_dict[town]['age_data'][age_col] = 0
    
    return population_data_dict

def merge_town_data_fixed():
    """修正版町丁データマージ処理"""
    try:
        print("=== 修正版町丁データマージ開始 ===")
        ...
        towns_file = output_dir / "consistent_towns_list_fixed.txt"
        
        with open(towns_file, 'w', encoding='utf-8') as f:
            f.write("=== 一貫性のある町丁一覧（修正版） ===\n")
        ...
        # サマリーファイルを作成
        summary_file = output_dir / "merge_summary_fixed.csv"
        summary_data = []
        ...
        for csv_file in sorted(csv_files):
            ...
            # 人口データをデータフレームに変換（総人口・男性・女性・年齢区分の順序を保持）
            population_data_list = []
            for town_name in town_names_in_filtered:
                if town_name in population_data_dict:
                    town_data = population_data_dict[town_name]
            ...
            # インデックスをリセットして行の対応関係を保つ
            filtered_df = filtered_df.reset_index(drop=True)
            population_df = population_df.reset_index(drop=True)
            
            # 元のデータフレームからColumn_1-3を除外して結合
            filtered_df_no_columns = filtered_df.iloc[:, :1]  # 最初の列（町丁名）のみ保持
            result_df = pd.concat([filtered_df_no_columns, population_df], axis=1)
            
            # データの整合性を確認
            print(f"  行数: {len(result_df)}町丁")
            
            # 出力ファイル名
            output_file = output_dir / f"{year_month}_consistent_fixed.csv"
...
def main():
    """メイン処理"""
    print("=== 修正版町丁データマージツール ===")
    
    print(f"\nこのツールは以下の処理を実行します:")
    print(f"1. 年齢区分やヘッダー行を除外")
    print(f"2. 有効な町丁名のみを抽出（区名付きも含む）")
    print(f"3. 町丁名の正規化（区名は保持、空白削除）")
