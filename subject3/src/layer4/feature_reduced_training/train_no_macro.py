# -*- coding: utf-8 -*-
# src/layer4/feature_reduced_training/train_no_macro.py
"""
...
# アブレーション研究のno_macroで除外する特徴量
MACRO_FEATURES_TO_REMOVE = [
    "macro_delta",
    "macro_excl", 
    "macro_ma3",
    "macro_shock"
...
def select_no_macro_features(df):
    """マクロ経済特徴量を除外して特徴量を選択（アブレーション研究のno_macroに基づく）"""
    print(f"除外するマクロ経済特徴量: {len(MACRO_FEATURES_TO_REMOVE)}個")
    
    # 除外対象特徴量を表示
    for feature in MACRO_FEATURES_TO_REMOVE:
        print(f"  - {feature}")
    
    # フルモデルと同じ特徴量選択ロジックを使用
    all_features = select_feature_columns(df)
    remaining_features = [f for f in all_features if f not in MACRO_FEATURES_TO_REMOVE]
    
    print(f"フルモデルの特徴量数: {len(all_features)}")
    print(f"除外後の特徴量数: {len(remaining_features)}")
    print(f"除外率: {len(MACRO_FEATURES_TO_REMOVE)/len(all_features)*100:.1f}%")
    ...
    if len(remaining_features) != expected_features:
        ...
        print(f"期待される除外特徴量: {MACRO_FEATURES_TO_REMOVE}")
...
def train_no_macro_model():
    """マクロ経済特徴量を除外したモデルを学習（アブレーション研究のno_macro再現）"""
    print("=" * 80)
    print("アブレーション研究のno_macro結果を再現")
    print("=" * 80)
    ...
    # マクロ経済特徴量を除外して特徴量を選択
    remaining_features = select_no_macro_features(df)
    ...
    # アブレーション研究のno_macroと同じフォールド設定
    # 1998-2018年を学習、2019年をテストから開始して7フォールド
    test_years_list = [2019, 2020, 2021, 2022, 2023, 2024, 2025]
    folds = []
    ...
    results = {
        ...
        'excluded_features': MACRO_FEATURES_TO_REMOVE,
        ...
        'feature_info': {
            'original_features': len(remaining_features) + len(MACRO_FEATURES_TO_REMOVE),
            'remaining_features': len(remaining_features),
            'excluded_features': len(MACRO_FEATURES_TO_REMOVE),
            'exclusion_rate': len(MACRO_FEATURES_TO_REMOVE) / (len(remaining_features) + len(MACRO_FEATURES_TO_REMOVE)) * 100,
            'excluded_features_list': MACRO_FEATURES_TO_REMOVE
        },
        'fold_metrics': fold_metrics,
        'aggregate_metrics': agg_metrics,
        'n_features_used': len(remaining_features),
        'n_features_excluded': len(MACRO_FEATURES_TO_REMOVE),
    ...
    # 結果を表示
    print(f"\n" + "=" * 80)
    print("アブレーション研究のno_macro結果再現完了")
    print("=" * 80)
    print(f"特徴量除外: {len(MACRO_FEATURES_TO_REMOVE)}個削除 ({results['feature_info']['exclusion_rate']:.1f}%)")
    print(f"集約性能:")
    ...
    # 各フォールドの結果も表示
    print(f"\n各フォールドの結果:")
    for i, (fold_metric, test_year) in enumerate(zip(fold_metrics, test_years_list), 1):
        print(f"  フォールド{i} ({test_year}年): MAE={fold_metric['MAE']:.4f}, R²={fold_metric['R2']:.4f}")
    
    return results

if __name__ == "__main__":
    train_no_macro_model()
